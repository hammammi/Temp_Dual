...
unsigned int cycle_ns = 1000000*2;
        ...
        if (ec_config_init(FALSE) > 0)
        {
            ...
            for (int k=2; k<(NUMOFMANI_DRIVE+2); ++k)
            {
                if (( ec_slavecount >= 1 ) && (strcmp(ec_slave[k].name,"EPOS4") == 0)) //change name for other drives
                {
                    ...
                    if (k<9){
                        wkc_count=ec_SDOwrite(k, 0x3142, 0x04, FALSE, os, &ob3, EC_TIMEOUTRXM); // for ver1
                    }
                    else{
                        wkc_count=ec_SDOwrite(k, 0x3142, 0x02, FALSE, os, &ob3, EC_TIMEOUTRXM); // for ver2
                    }
...
void EPOS_OP(void *arg)
{
    ...
    for (i=0; i<NUMOFEPOS4_DRIVE; i++)
    {
        epos4_drive_pt[i].ptOutParam->ControlWord=6; //2?Servo OFF (Disable voltage, transition#9)
    }
    ec_send_processdata();
    wkc = ec_receive_processdata(EC_TIMEOUTRET);
