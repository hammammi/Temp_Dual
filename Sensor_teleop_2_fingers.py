...
ser = serial.Serial(port = '/dev/ttyUSB5',baudrate=115200)
ser2  = serial.Serial(port = '/dev/ttyUSB6',baudrate=115200)

def thread_run():
	global value, value2, pub2, torque, torque2
...
def talker():
	global value, value2, pub2, torque, torque2, iteration, torque_sum, encoder_baseline, torque_sum2, encoder_baseline2
...
#	thread_run()
	while not rospy.is_shutdown():
		...
		if response2.__len__() == n_ch*4+1:
			# Putting in the 6 values into data
			# Last byte not taken
			data2 = np.array(struct.unpack('<lllll', response2[:-1]))
